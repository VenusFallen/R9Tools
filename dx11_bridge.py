"""
Direct3D 11 / DXGI COM interface wrappers for the imgui overlay renderer.
Uses ctypes vtable calls — no extra dependencies beyond the Windows OS.

Only the exact methods we use are implemented.  Vtable offsets are fixed
by the Windows SDK ABI and do not change across D3D11 versions.
"""
import ctypes
from ctypes import (
...
def _check(hr: int, label: str = "D3D11"):
    if hr < 0:
...
def _query(ptr: int, iid: GUID) -> int:
    out = c_void_p(None)
    hr  = _com(ptr, 0, c_int, [POINTER(GUID), POINTER(c_void_p)],
               byref(iid), byref(out))
...
def create_device() -> tuple[int, int]:
    ...
    BGRA_SUPPORT flag is set — required for B8G8R8A8 render targets used by DComp.
    Returns (ID3D11Device*, ID3D11DeviceContext*) as raw integer pointers.
    """
...
def get_factory2() -> int:
    """
    Create an IDXGIFactory2 directly via CreateDXGIFactory2 (DXGI 1.3, Windows 8.1+).
    Returns IDXGIFactory2* (caller must Release).

    This is simpler and more reliable than walking device→IDXGIDevice→adapter→GetParent.
    D3D_DRIVER_TYPE_HARDWARE always uses adapter 0, which CreateDXGIFactory2 also
    enumerates first — so there is no adapter mismatch on single-GPU systems.
    """
...
def create_swap_chain_for_composition(factory2: int, device: int,
...
def swap_present(swap_chain: int, sync_interval: int = 1, flags: int = 0) -> int:
...
def swap_resize(swap_chain: int, width: int, height: int):
...
def ctx_set_rtv(context: int, rtv: int):
    rtv_arr = (c_void_p * 1)(rtv)
    _com(context, 33, None,
         [c_uint, POINTER(c_void_p * 1), c_void_p],
         1, rtv_arr, None)


def ctx_clear_rtv(context: int, rtv: int, r: float, g: float, b: float, a: float):
    col = (c_float * 4)(r, g, b, a)
    _com(context, 50, None,
         [c_void_p, c_float * 4],
         c_void_p(rtv), col)


def ctx_set_viewport(context: int, width: float, height: float):
    vp     = D3D11Viewport(0, 0, width, height, 0.0, 1.0)
    vp_arr = (D3D11Viewport * 1)(vp)
    _com(context, 44, None,
         [c_uint, POINTER(D3D11Viewport * 1)],
         1, vp_arr)
